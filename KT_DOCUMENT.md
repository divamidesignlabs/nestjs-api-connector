...
### 3. Customizing the Table Name

If you already have a table or want to use a different name instead of `connector_mappings_config`:
...
const MyCustomEntity = getMappingEntity('my_custom_table_name');
...
@Module({
  imports: [
    ...
    ConnectorModule.forRootAsync({
      inject: [DataSource],
      useFactory: (dataSource: DataSource) => ({
        tableName: 'my_custom_table_name', // Optional: for reference
...
3.  **Scale**: Add more connectors to the `connector_mappings_config` table.
